...
import fetch, { AbortError } from 'node-fetch'; // Import AbortError for timeout handling
...
async function writeFile(args: { filepath: string, content: string }): Promise<string> {
...
async function runShellCommand(args: { command: string }): Promise<string> {
    try {
...
const AVAILABLE_TOOLS: { [key: string]: (args: any) => Promise<string> } = {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerCommand('agent-connector.runAgent', async (userPrompt: string) => {
        if (!userPrompt) {
        ...
        for (let i = 0; i < MAX_TURNS; i++) {
            ...
            let response;
            try {
                response = await fetch(AGENT_DECISION_URL, {
                    method: 'POST',
